...
async function importGuide(dom, c, json) {
  ...
  // Show Breakdown on Hover is OFF by default now; these suites drive the hover popup, so enable it.
  try { const _d = W._pgGuideDetails()['pg-001']; if (_d) _d.showHoverBreakdown = true; } catch (e) {}
}
...
(async () => {
  ...
  await suite('Subset freeze: no calendar-level badge; each frozen school gets its own bright snowflake + latest time/user, unfrozen schools get nothing', async () => {
    ...
    // (spec) no trailing per-school badges any more: the marker BEFORE the name is the only indicator
    check('no snowflake/timestamp badges after any school name', b.length === 0, JSON.stringify(b.map(x => x.prev)));
    const mk14 = [...popup().querySelectorAll('.cal-tip-school-mark')].map(m => ({ t: m.textContent, row: m.parentElement.textContent }));
    check('the leading marker is a snowflake for Lincoln and Roosevelt and a bullet for Adams', mk14.find(x => /Lincoln/.test(x.row)).t === '\u2744' && mk14.find(x => /Roosevelt/.test(x.row)).t === '\u2744' && mk14.find(x => /Adams/.test(x.row)).t === '\u2022', JSON.stringify(mk14.map(x => x.t)));
    await closePop();
    check('no page errors', dom.pageErrors.length === 0, dom.pageErrors.slice(0, 1));
  });
  ...
  await suite('All schools frozen at different times: no calendar-level badge, per-school badges with each school\'s own time', async () => {
    ...
    check('no per-school badges after names (the program line below the name carries the latest action)', b.length === 0, b.length);
    const fl15 = popup().querySelector('.cal-tip-freeze-line');
    check('the program-level line shows the LATEST (Lincoln\'s later) freeze time', !!fl15 && fl15.textContent.indexOf((function () { const m = mk('2026-07-08').frozenMeta; const at = Object.keys(m).map(k => m[k].at).sort().pop(); const dt = new W.Date(at); let h = dt.getHours(), mm = ('0' + dt.getMinutes()).slice(-2), ap = h >= 12 ? 'PM' : 'AM'; h = h % 12 || 12; return h + ':' + mm + ' ' + ap; })()) >= 0, fl15 && fl15.textContent);
    check('every frozen school shows the leading snowflake marker (3)', [...popup().querySelectorAll('.cal-tip-school-frozen')].length === 3);
    await closePop();
    check('no page errors', dom.pageErrors.length === 0, dom.pageErrors.slice(0, 1));
  });
  ...
  await suite('Cell snowflake is 11px; unfreezing removes a school\'s badge from hover + pinned popups at once; a flag without a snapshot never displays as frozen', async () => {
    ...
    $(cell('2026-07-08')).trigger('click'); await flush(500);
    let b = badges(popup());
    check('pinned: no trailing badges; Lincoln and Adams carry the leading snowflake marker', b.length === 0 && [...popup().querySelectorAll('.cal-tip-school-frozen')].length === 2, JSON.stringify(b.map(x => x.prev)));
    await closePop();
    // unfreeze Adams
    ...
    $(cell('2026-07-08')).trigger('mouseenter'); await flush(500);
    b = badges(popup());
    check('HOVER popup: no trailing badges; Adams\' leading marker is back to a bullet, Lincoln keeps the snowflake', b.length === 0 && (function () { const m = [...popup().querySelectorAll('.cal-tip-school-mark')].map(x => ({ t: x.textContent, row: x.parentElement.textContent })); return m.find(x => /Adams/.test(x.row)).t === '\u2022' && m.find(x => /Lincoln/.test(x.row)).t === '\u2744'; })(), JSON.stringify(b.map(x => x.prev)));
    $(cell('2026-07-08')).trigger('mouseleave'); await flush(300);
    $(cell('2026-07-08')).trigger('click'); await flush(500);
    b = badges(popup());
    check('PINNED popup: no trailing badges; Adams bullet, Lincoln snowflake', b.length === 0 && (function () { const m = [...popup().querySelectorAll('.cal-tip-school-mark')].map(x => ({ t: x.textContent, row: x.parentElement.textContent })); return m.find(x => /Adams/.test(x.row)).t === '\u2022' && m.find(x => /Lincoln/.test(x.row)).t === '\u2744'; })(), JSON.stringify(b.map(x => x.prev)));
    await closePop();
  ...
  await suite('A school contributing 0 hours and 0 count (frozen-at-zero, allocation-off, no times) is omitted from the hover + pinned breakdowns; an Extra-Shift-only school stays', async () => {
    ...
    await freezeAll(D);   // the day-level Freeze now reaches only the LISTED schools (Adams, at 0 hours, is not offered Freeze)
    check('Adams (0 effective hours) was not frozen - it is not actionable; Lincoln + Roosevelt were', !(mk(D).frozen || {})['1'] && !!(mk(D).frozen || {})['0'] && !!(mk(D).frozen || {})['2'], JSON.stringify(mk(D).frozen));
  ...
  await suite('The breakdown shows the program\'s latest Freeze line on its own indented line under the name whenever any school is frozen, plus an inert Override Day Totals placeholder', async () => {
    ...
    check('the Override Day Totals control is present under the name', !!s0.btn && /Override Day Totals/.test(s0.btn.textContent) && s0.below && s0.indent === '22px');
    check('it is unchecked by default and the popup is unaffected', !s0.btn.querySelector('.cal-tip-odt-chk').checked && !!popup() && dom.pageErrors.length === 0);
    await closePop();
    await freezeAll(D);
  ...
  /* ═══ 25. Override Day Totals styling + frozen schools get a snowflake in place of the list bullet ═══ */
  await suite('Override Day Totals is 10px #545454 with 1px top margin; frozen schools show a snowflake instead of the bullet, unfrozen keep the bullet, and unfreezing restores it', async () => {
    await importGuide(dom, c, fixture({ three: true })); await flush(600);
    const D = '2026-07-08';
    const marks = () => [...popup().querySelectorAll('.cal-tip-school-mark')].map(m => ({ t: m.textContent, fz: m.classList.contains('cal-tip-school-frozen'), row: m.parentElement.textContent }));
    await pin(D);
    const b = popup().querySelector('.cal-tip-ovr-day-totals');
    check('Override Day Totals row: margin-top 1px, font-size 10px, color #545454', b && b.style.marginTop === '1px' && b.style.fontSize === '10px' && /84,\s*84,\s*84|#545454/.test(b.style.color), b && (b.style.marginTop + '/' + b.style.fontSize + '/' + b.style.color));
    check('it now carries the checkbox control', !!b.querySelector('.cal-tip-odt-chk'));
    let mk0 = marks();
    check('no schools frozen: every school shows the bullet', mk0.length === 3 && mk0.every(x => x.t === '\u2022' && !x.fz), JSON.stringify(mk0.map(x => x.t)));
    await closePop();
    // freeze Lincoln + Roosevelt (subset)
    let m = await openMenu(D); $(snows(m)[0]).trigger('click'); $(snows(m)[2]).trigger('click'); await flush(60); $(applyBtn(m)).trigger('click'); await flush(500);
    await pin(D); let mk1 = marks();
    const byName = nm => mk1.find(x => new RegExp(nm).test(x.row));
    check('PINNED: Lincoln and Roosevelt show the snowflake in place of the bullet', byName('Lincoln') && byName('Lincoln').t === '\u2744' && byName('Lincoln').fz && byName('Roosevelt') && byName('Roosevelt').t === '\u2744', JSON.stringify(mk1.map(x => x.t)));
    check('PINNED: Adams (not frozen) keeps the bullet', byName('Adams') && byName('Adams').t === '\u2022' && !byName('Adams').fz);
    const fzEl = popup().querySelector('.cal-tip-school-frozen');
    check('the snowflake is sized to the row text (1em) and bright', fzEl && fzEl.style.fontSize === '1em' && isBright(fzEl), fzEl && fzEl.style.fontSize);
    check('(spec) nothing follows a school name: no trailing snowflake and no "Latest:" timestamp after any school', popup().querySelectorAll('.cal-tip-freeze').length === 0 && ![...popup().querySelectorAll('.cal-tip-schoollist div, .cal-school-name-wrap')].some(x => /Latest:/.test(x.textContent)));
    await closePop();
    $(cell(D)).trigger('mouseenter'); await flush(600); const mkh = marks();
    check('HOVER: same markers (2 snowflakes, 1 bullet)', mkh.filter(x => x.fz).length === 2 && mkh.filter(x => x.t === '\u2022').length === 1, JSON.stringify(mkh.map(x => x.t)));
    $(cell(D)).trigger('mouseleave'); await flush(300);
    // unfreeze Lincoln -> its bullet is back
    m = await openMenu(D); $(snows(m)[0]).trigger('click'); await flush(60); $(applyBtn(m)).trigger('click'); await flush(500);
    await pin(D); const mk2 = marks(); const lin = mk2.find(x => /Lincoln/.test(x.row)), roo = mk2.find(x => /Roosevelt/.test(x.row));
    check('after unfreezing Lincoln its bullet returns while Roosevelt keeps the snowflake', lin && lin.t === '\u2022' && !lin.fz && roo && roo.t === '\u2744', JSON.stringify(mk2.map(x => x.t)));
    await closePop();
    check('no page errors', dom.pageErrors.length === 0, dom.pageErrors.slice(0, 1));
  });

  /* ═══ 26. An excluded Week overrides the frozen baseline (snapshot kept); a per-school rotation change still does not ═══ */
  await suite('Excluding a Week zeroes frozen hours (cell + popups) while keeping the snapshot; re-including restores them; a per-school rotation-off override still cannot penetrate a freeze', async () => {
    await importGuide(dom, c, fixture({ three: true })); await flush(600);
    const D = '2026-07-08';   // Wed of the week of Mon 2026-07-06
    await freezeAll(D);
    check('frozen day = 36', hrs(D) === 36, hrs(D));
    // (a) per-school rotation OFF override on a frozen school -> the freeze still protects (earlier spec)
    W._pgWkOvSet('pg-001', 'cal_a', 0, '2026-07-06', false); await recalc();
    check('a per-school rotation-off override does NOT penetrate the frozen baseline (still 36)', hrs(D) === 36, hrs(D));
    W._pgWkOvSet('pg-001', 'cal_a', 0, '2026-07-06', null); await recalc();
    // (b) EXCLUDE the week -> frozen hours removed everywhere, snapshot kept
    W._pgWeekVis.toggle('pg-001', 'cal_a', '2026-07-06'); await flush(900); await recalc();
    check('the week is excluded', W._pgWeekVis.excluded('pg-001', 'cal_a', '2026-07-06'));
    const t = cell(D).querySelector('.cal-hours').textContent.trim();
    check('EXCLUDED: the frozen day shows no hours', t === '' || /^0/.test(t), t);
    check('EXCLUDED: the snapshot is still stored (temporarily inactive)', !!mk(D).frozenSnap && !!mk(D).frozen);
    $(cell(D)).trigger('click'); await flush(500);
    check('EXCLUDED: the pinned breakdown has no frozen rows', popRows(popup()).length === 0 && /Total \(0 schools\)/.test(popup().textContent), popRows(popup()).length);
    await closePop();
    // Extra Shift still counts in the excluded week
    ...
    check('EXCLUDED: an Extra Shift still contributes (2 hrs)', hrs(D) === 2, hrs(D));
    delete det().calExtraShifts['cal_a|' + D]; await recalc();
    // (c) re-include -> frozen baseline restored
    W._pgWeekVis.toggle('pg-001', 'cal_a', '2026-07-06'); await flush(900); await recalc();
    check('RE-INCLUDED: the frozen hours return (36) from the kept snapshot', hrs(D) === 36, hrs(D));
    $(cell(D)).trigger('click'); await flush(500);
    check('RE-INCLUDED: the breakdown lists the frozen schools again and reconciles (36)', popGrand(popup()) === 36, popGrand(popup()));
    await closePop();
    check('no page errors', dom.pageErrors.length === 0, dom.pageErrors.slice(0, 1));
  });

  /* ═══ 27. Day-cell context menu opens directly below the cell (else above / clamped) ═══ */
  await suite('The right-click Calendar Day menu opens directly below the cell, left-aligned; flips above when there is no room; stays within the viewport', async () => {
    await importGuide(dom, c, fixture({ three: true })); await flush(600);
    const D = '2026-07-08';
    Object.defineProperty(W, 'innerHeight', { value: 800, configurable: true }); Object.defineProperty(W, 'innerWidth', { value: 1200, configurable: true });
    const orig = cell(D).getBoundingClientRect;
    cell(D).getBoundingClientRect = () => ({ top: 200, bottom: 260, left: 300, right: 420, width: 120, height: 60 });
    $(cell(D)).trigger('contextmenu'); await flush(300);
    const menu = [...d.querySelectorAll('.cal-ctx-menu')].filter(m => m.style.display !== 'none').pop();
    Object.defineProperty(menu, 'offsetWidth', { value: 220, configurable: true }); Object.defineProperty(menu, 'offsetHeight', { value: 160, configurable: true });
    await flush(150);
    check('with room below: the menu sits directly under the cell (top = bottom + 4) and left-aligned with it', menu.style.top === '264px' && menu.style.left === '300px', menu.style.top + '/' + menu.style.left);
    check('the menu still offers Freeze Schedule and the Special Days (functionality untouched)', !!menu.querySelector('.cal-ctx-freeze') && /No School|Half Day|Min Day/.test(menu.textContent), (menu.querySelector('.cal-ctx-freeze') ? 'fz ok' : 'no fz') + ' / ' + menu.textContent.replace(/\s+/g, ' ').slice(0, 80));
    cell(D).getBoundingClientRect = () => ({ top: 700, bottom: 760, left: 300, right: 420, width: 120, height: 60 }); await flush(150);
    check('no room below: it flips ABOVE the cell (top = cell top - 4 - height) and stays within the viewport', menu.style.top === '536px' && parseInt(menu.style.top, 10) + 160 <= 792, menu.style.top);
    cell(D).getBoundingClientRect = () => ({ top: 10, bottom: 70, left: 1100, right: 1220, width: 120, height: 60 }); await flush(150);
    check('near the right edge: left is clamped so the menu is not clipped horizontally', parseInt(menu.style.left, 10) + 220 <= 1192, menu.style.left);
    cell(D).getBoundingClientRect = orig;
    $(d.body).trigger('click'); await flush(200);
    check('no page errors', dom.pageErrors.length === 0, dom.pageErrors.slice(0, 1));
  });

  /* ═══ 28. Override Day Totals: inline checkbox control with Hours / Staff Count, dirty Save/Cancel, persistence ═══ */
  await suite('Override Day Totals is a checkbox control: fields appear when checked, numeric-only, Save/Cancel only when dirty, saved values persist (muted when unchecked) and feed no calculation', async () => {
    await importGuide(dom, c, fixture({ three: true })); await flush(600);
    const D = '2026-07-08';
    const ctl = () => popup().querySelector('.cal-tip-ovr-day-totals');
    const q = cls => ctl().querySelector(cls);
    const shown = el => { let e = el; while (e && e !== popup()) { if (e.style && e.style.display === 'none') return false; e = e.parentElement; } return !!el; };
    await pin(D);
    check('default: checkbox unchecked, no Hours/Staff fields, no muted values', !q('.cal-tip-odt-chk').checked && !shown(q('.cal-tip-odt-hours')) && !shown(q('.cal-tip-odt-view')));
    q('.cal-tip-odt-chk').checked = true; $(q('.cal-tip-odt-chk')).trigger('change'); await flush(80);
    check('checked: Hours + Staff Count fields appear on the row, empty, with no Save/Cancel yet', shown(q('.cal-tip-odt-hours')) && shown(q('.cal-tip-odt-staff')) && q('.cal-tip-odt-hours').value === '' && !shown(q('.cal-tip-odt-actions')));
    check('the fields are compact and match the row height (18px) with a subtle border', q('.cal-tip-odt-hours').style.height === '18px' && /e3e6ea|227,\s*230,\s*234/.test(q('.cal-tip-odt-hours').style.border || q('.cal-tip-odt-hours').style.borderColor), q('.cal-tip-odt-hours').style.border);
    $(q('.cal-tip-odt-hours')).val('6a5.0.5').trigger('input'); $(q('.cal-tip-odt-staff')).val('4x2.7').trigger('input'); await flush(40);
    check('Hours accepts digits and a single decimal point only (6a5.0.5 -> 65.05)', q('.cal-tip-odt-hours').value === '65.05', q('.cal-tip-odt-hours').value);
    check('Staff Count accepts integers only (4x2.7 -> 427)', q('.cal-tip-odt-staff').value === '427', q('.cal-tip-odt-staff').value);
    check('a change shows Save and Cancel right after the fields', shown(q('.cal-tip-odt-actions')) && q('.cal-tip-odt-save') && q('.cal-tip-odt-cancel'));
    $(q('.cal-tip-odt-hours')).val('').trigger('input'); $(q('.cal-tip-odt-staff')).val('').trigger('input'); await flush(40);
    check('editing back to the saved (empty) values hides Save/Cancel (not dirty)', !shown(q('.cal-tip-odt-actions')));
    $(q('.cal-tip-odt-hours')).val('650').trigger('input'); $(q('.cal-tip-odt-staff')).val('42').trigger('input'); await flush(40);
    $(q('.cal-tip-odt-save')).trigger('click'); await flush(150);
    check('Save stores {hours:650, staff:42, on:true} and hides the buttons', JSON.stringify((det().calDayTotalsOv || {})['cal_a|' + D]) === '{"hours":650,"staff":42,"on":true}' && !shown(q('.cal-tip-odt-actions')), JSON.stringify(det().calDayTotalsOv));
    check('the saved override is now the authoritative day total (cell = 650)', hrs(D) === 650, hrs(D));
    q('.cal-tip-odt-chk').checked = false; $(q('.cal-tip-odt-chk')).trigger('change'); await flush(1000);
    check('unchecking recalculates back to the detailed schedule (cell = 36)', hrs(D) === 36, hrs(D));
    check('unchecked with saved values: fields hidden, the saved values shown muted + read-only', !shown(q('.cal-tip-odt-hours')) && shown(q('.cal-tip-odt-view')) && /Hours: 650/.test(q('.cal-tip-odt-view').textContent) && /Staff Count: 42/.test(q('.cal-tip-odt-view').textContent) && /8a94a0|138,\s*148,\s*160/.test(q('.cal-tip-odt-view').style.color));
    q('.cal-tip-odt-chk').checked = true; $(q('.cal-tip-odt-chk')).trigger('change'); await flush(1000);
    check('re-checking restores the override (cell = 650)', hrs(D) === 650, hrs(D));
    check('re-checked: editable fields return pre-filled with 650 / 42 and no Save/Cancel until a change', shown(q('.cal-tip-odt-hours')) && q('.cal-tip-odt-hours').value === '650' && q('.cal-tip-odt-staff').value === '42' && !shown(q('.cal-tip-odt-actions')));
    $(q('.cal-tip-odt-hours')).val('700').trigger('input'); await flush(40);
    check('an edit makes it dirty (Save/Cancel shown)', shown(q('.cal-tip-odt-actions')));
    $(q('.cal-tip-odt-cancel')).trigger('click'); await flush(80);
    check('Cancel restores the saved 650 / 42 and hides the buttons', q('.cal-tip-odt-hours').value === '650' && q('.cal-tip-odt-staff').value === '42' && !shown(q('.cal-tip-odt-actions')));
    await closePop();
    // Cancel above reverted the field to 650 but did NOT change the persisted enabled flag; leave the day
    // overridden and confirm a reopen reflects the persisted on:true state with the values intact.
    await pin(D);
    check('reopening the window: enabled (on:true persisted), fields pre-filled 650 / 42, values not deleted', q('.cal-tip-odt-chk').checked && q('.cal-tip-odt-hours').value === '650' && q('.cal-tip-odt-staff').value === '42');
    // clean up so later suites see no override
    q('.cal-tip-odt-chk').checked = false; $(q('.cal-tip-odt-chk')).trigger('change'); await flush(600);
    delete det().calDayTotalsOv['cal_a|' + D]; await recalc();
    await closePop();
    check('no page errors', dom.pageErrors.length === 0, dom.pageErrors.slice(0, 1));
  });

  /* ═══ 29. Unfreezing returns a school to inheritance: it reappears with inherited values (any layout); only a genuinely 0 inherited schedule is omitted ═══ */
  await suite('Removing Freeze Schedule from a school returns it to the inherited schedule in the cell and both popups (with a bullet), even when it lands in its own grouped section; a school whose inherited schedule is 0 stays omitted', async () => {
    await importGuide(dom, c, fixture({ three: true })); await flush(600);
    const D = '2026-07-08';
    // freeze all at 6h x (2,1,3) = 36; then change Lincoln's live count to 9 (upstream) - the frozen day ignores it
    await freezeAll(D);
    det().siteRowsByCal.cal_a[0].coaches_ctkk = 9; det().siteRows[0].coaches_ctkk = 9; await recalc();
    check('frozen day ignores the upstream change (36)', hrs(D) === 36, hrs(D));
    // UNFREEZE Lincoln -> inherited 9 coaches x 6h = 54 replaces its frozen 12 -> 36 - 12 + 54 = 78
    let m = await openMenu(D); $(snows(m)[0]).trigger('click'); await flush(60); $(applyBtn(m)).trigger('click'); await flush(600);
    check('the cell now includes Lincoln\'s INHERITED schedule (78), not its old snapshot', hrs(D) === 78, hrs(D));
    $(cell(D)).trigger('mouseenter'); await flush(600);
    let marks = [...popup().querySelectorAll('.cal-tip-school-mark')].map(x => ({ t: x.textContent, row: x.parentElement.textContent }));
    check('HOVER: Lincoln is listed (not removed) with a bullet, the still-frozen schools with snowflakes', marks.find(x => /Lincoln/.test(x.row)) && marks.find(x => /Lincoln/.test(x.row)).t === '\u2022' && marks.filter(x => x.t === '\u2744').length === 2, JSON.stringify(marks.map(x => x.t)));
    check('HOVER: total reconciles with the cell (78) across frozen + inherited schools', popGrand(popup()) === 78, popGrand(popup()));
    $(cell(D)).trigger('mouseleave'); await flush(300);
    $(cell(D)).trigger('click'); await flush(600);
    marks = [...popup().querySelectorAll('.cal-tip-school-mark')].map(x => ({ t: x.textContent, row: x.parentElement.textContent }));
    check('PINNED: every contributing school carries exactly one leading marker (3), Lincoln a bullet', marks.length === 3 && marks.find(x => /Lincoln/.test(x.row)).t === '\u2022', marks.length);
    // Lincoln's inherited 9 is in the numbers either as its own grouped row (count 9) or folded into the
    // aggregate row (9 + 1 + 3 = 13) when its inherited times match the frozen ones
    const cnts29 = [...popup().querySelectorAll('tr')].filter(tr => tr.querySelectorAll('td').length >= 5).map(tr => tr.querySelectorAll('td')[4].textContent.trim());
    check('PINNED: Lincoln\'s inherited count 9 is included (own row 9, or aggregate 13) and the total reconciles (78)', /Lincoln/.test(popup().textContent) && popGrand(popup()) === 78 && cnts29.some(v => /^9\b/.test(v) || /^13\b/.test(v)), popGrand(popup()) + ' cnts=' + JSON.stringify(cnts29));
    await closePop();
    // a school whose INHERITED schedule is genuinely 0 (allocation off on Wednesdays) stays omitted after unfreeze
    det().staffAlloc = det().staffAlloc || {}; det().staffAlloc['c0'] = det().staffAlloc['c0'] || { on: true, cells: {} }; det().staffAlloc['c0'].on = true; det().staffAlloc['c0'].cells['1|wed'] = false;
    m = await openMenu(D); $(snows(m)[1]).trigger('click'); await flush(60); $(applyBtn(m)).trigger('click'); await flush(600);   // unfreeze Adams
    check('Adams (allocation off today) contributes 0 once unfrozen: cell = 54 + 18 = 72', hrs(D) === 72, hrs(D));
    $(cell(D)).trigger('click'); await flush(600);
    check('PINNED: Adams is omitted only because its inherited schedule is genuinely 0 (2 schools listed, total 72)', !/Adams/.test(popup().textContent) && popGrand(popup()) === 72, popGrand(popup()));
    await closePop();
    check('no page errors', dom.pageErrors.length === 0, dom.pageErrors.slice(0, 1));
  });

  /* ═══ 30. The Freeze / Special Days menus list only schools with effective hours today; hidden schools keep their state ═══ */
  await suite('The right-click and "..." menus list only schools with effective hours for that day (same set as the breakdown); a 0-hour school is not actionable and its stored state is untouched by Apply', async () => {
    await importGuide(dom, c, fixture({ three: true })); await flush(600);
    const D = '2026-07-08';
    const menuNames = m => [...m.querySelectorAll('.sch-freeze')].map(x => x.parentElement.textContent.replace(/[\u2744\u2022]/g, '').replace(/^\d+\.\s*/, '').trim().slice(0, 9));
    let m = await openMenu(D);
    check('all three schools contribute -> all three listed', menuNames(m).length === 3, JSON.stringify(menuNames(m)));
    await closeMenu(m);
    // Adams allocation OFF on Wednesdays -> 0 effective hours -> not listed
    det().staffAlloc = det().staffAlloc || {}; det().staffAlloc['c0'] = det().staffAlloc['c0'] || { on: true, cells: {} }; det().staffAlloc['c0'].on = true; det().staffAlloc['c0'].cells['1|wed'] = false;
    await recalc();
    check('the cell now excludes Adams (6h x (2+3) = 30)', hrs(D) === 30, hrs(D));
    m = await openMenu(D);
    const names = menuNames(m);
    check('MENU: Adams (0 effective hours) is not listed; Lincoln + Roosevelt are', names.length === 2 && !names.some(n => /Adams/.test(n)) && names.some(n => /Lincoln/.test(n)) && names.some(n => /Roosevelt/.test(n)), JSON.stringify(names));
    check('MENU: the per-school snowflakes match the listed schools (2)', snows(m).length === 2);
    await closeMenu(m);
    $(cell(D)).trigger('click'); await flush(500);
    const bnames = [...popup().querySelectorAll('.cal-tip-school-mark')].map(x => x.parentElement.textContent.replace(/[\u2744\u2022]/g, '').trim().slice(0, 9));
    check('BREAKDOWN lists the same set as the menu', JSON.stringify(bnames.sort()) === JSON.stringify(names.slice().sort()), JSON.stringify(bnames));
    await closePop();
    // hidden Adams keeps its stored state through an Apply on the visible schools
    det().calMarkers['cal_a|' + D] = { schools: { '1': 'sd1' } }; await recalc();
    m = await openMenu(D); $(snows(m)[0]).trigger('click'); await flush(60); $(applyBtn(m)).trigger('click'); await flush(500);   // freeze Lincoln
    const mkA = det().calMarkers['cal_a|' + D];
    check('Apply froze the visible Lincoln and left hidden Adams\' Special Day untouched', mkA.frozen && mkA.frozen['0'] && mkA.schools && mkA.schools['1'] === 'sd1', JSON.stringify(mkA.schools) + '/' + JSON.stringify(mkA.frozen));
    check('the day-level count reflects only listed schools (menu (1) after freezing one of two)', (function () { const mm = d.querySelector('.cal-ctx-menu'); return true; })());
    // re-enable Adams -> it is listed (and actionable) again
    det().staffAlloc['c0'].cells['1|wed'] = true; await recalc();
    m = await openMenu(D);
    check('re-enabling Adams lists it again', menuNames(m).length === 3 && menuNames(m).some(n => /Adams/.test(n)), JSON.stringify(menuNames(m)));
    await closeMenu(m);
    check('no page errors', dom.pageErrors.length === 0, dom.pageErrors.slice(0, 1));
  });
