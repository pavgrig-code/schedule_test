...
(async () => {
  ...
  await suite('Override Day Totals is a checkbox control: fields appear when checked, numeric-only, Save/Cancel only when dirty, saved values persist (muted when unchecked) and feed no calculation', async () => {
    ...
    check('Save stores {hours:650, staff:42} for this calendar/date and hides the buttons', JSON.stringify((det().calDayTotalsOv || {})['cal_a|' + D]) === '{"hours":650,"staff":42}' && !shown(q('.cal-tip-odt-actions')), JSON.stringify(det().calDayTotalsOv));
    check('the saved override feeds NO calculation (cell still 36)', hrs(D) === 36, hrs(D));
    q('.cal-tip-odt-chk').checked = false; $(q('.cal-tip-odt-chk')).trigger('change'); await flush(80);
    ...
    await closePop();
    await pin(D);
    check('reopening the window: still unchecked, saved values shown muted (not deleted)', !q('.cal-tip-odt-chk').checked && shown(q('.cal-tip-odt-view')) && /650/.test(q('.cal-tip-odt-view').textContent));
    await closePop();
    check('no page errors', dom.pageErrors.length === 0, dom.pageErrors.slice(0, 1));
  });
