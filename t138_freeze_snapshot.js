...
(async () => {
  ...
  await suite('Whole-day uniform freeze: the popups show ONE bright snowflake + subtle latest time/user right after the Calendar name, none per school', async () => {
    ...
    check('hover: exactly one badge, at calendar level', b.length === 1 && b[0].where === 'CAL', JSON.stringify(b.map(x => x.where)));
    check('hover: it sits immediately after the Calendar/Program name', /CalA/.test(b[0].prev), b[0].prev);
    check('hover: it reads "❄ Latest: <Dow>, <Mon> <d>, <h:mm AM/PM> by Paul"', /^\u2744\s*Latest: \w{3}, \w{3} \d{1,2}, \d{1,2}:\d{2} (AM|PM) by Paul$/.test(b[0].text), b[0].text);
    check('hover: the snowflake is bright (#0d6efd) and bold', isBright(b[0].ico) && b[0].ico.style.fontWeight === '700', b[0].ico && b[0].ico.style.color);
    check('hover: the time/user text is subtle (small, grey)', b[0].meta && parseInt(b[0].meta.style.fontSize, 10) <= 9 && /8a94a0|138,\s*148,\s*160/.test(b[0].meta.style.color), b[0].meta && (b[0].meta.style.fontSize + '/' + b[0].meta.style.color));
    $(cell('2026-07-08')).trigger('mouseleave'); await flush(400);
    // pinned
    $(cell('2026-07-08')).trigger('click'); await flush(500);
    pop = popup(); b = badges(pop);
    check('pinned: one calendar-level badge after the name, no school badges', b.length === 1 && b[0].where === 'CAL' && /CalA/.test(b[0].prev), JSON.stringify(b.map(x => x.where + ':' + x.prev)));
    await closePop();
    check('no page errors', dom.pageErrors.length === 0, dom.pageErrors.slice(0, 1));
  });
  ...
  await suite('Subset freeze: no calendar-level badge; each frozen school gets its own bright snowflake + latest time/user, unfrozen schools get nothing', async () => {
    ...
    check('no calendar-level badge', !b.some(x => x.where === 'CAL'));
  ...
  await suite('All schools frozen at different times: no calendar-level badge, per-school badges with each school\'s own time', async () => {
    ...
    $(cell('2026-07-08')).trigger('click'); await flush(500);
    const pop = popup(); const b = badges(pop);
    check('differing times -> no calendar-level badge', !b.some(x => x.where === 'CAL'), JSON.stringify(b.map(x => x.where)));
