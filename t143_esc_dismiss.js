...
(async () => {
  ...
  await suite('The Freeze/Special Days menu closes on ANY outside click: another day cell, the same cell, the calendar area, the page body; a click inside keeps it open', async () => {
    await importGuide(dom, c, fixture()); await flush(600);
    ...
    const open = async dk => { $(cellOf(dk)).trigger('contextmenu'); await flush(300); };
    const clickOn = el => { ['mousedown', 'mouseup', 'click'].forEach(t => el.dispatchEvent(new W.MouseEvent(t, { bubbles: true, cancelable: true, view: W, button: 0 }))); };
    await open('2026-07-08'); check('menu opens', !!menu());
    clickOn(cellOf('2026-07-09')); await flush(200);
    check('clicking ANOTHER day cell closes it', !menu());
    await open('2026-07-08'); await flush(80); clickOn(cellOf('2026-07-08')); await flush(200);
    check('clicking the SAME cell (outside the menu) closes it', !menu());
    await open('2026-07-08'); await flush(80); clickOn(pl().querySelector('table') || pl()); await flush(200);
    check('clicking another calendar area closes it', !menu());
    await open('2026-07-08'); await flush(80); clickOn(d.body); await flush(200);
    check('clicking the page body closes it', !menu());
    await open('2026-07-08'); await flush(80); const inside = menu() && menu().querySelector('*'); if (inside) clickOn(inside); await flush(200);
    check('clicking INSIDE the menu keeps it open', !!menu());
    $(d.body).trigger('keydown', { key: 'Escape' });
    check('no page errors', dom.pageErrors.length === 0, dom.pageErrors.slice(0, 1));
  });
